...
{
#if !WIN32
    #if FW_START_BY_BOOTLOADER_EN   /*安信可不使用*/
    u32 fw_adr = DUAL_MODE_FW_ADDR_SIGMESH;
    #else
    u32 fw_adr = ota_program_offset ? 0 : 0x40000;/* 不知什么原因ota_program_offset永远等于0x40000？？？？？？*/
    LOG_USER_MSG_INFO(0, 0,"ota_program_offset:0x%x;fw_adr:0x%x",ota_program_offset,fw_adr);

    #endif
    flash_read_page(fw_adr+2, sizeof(fw_id_local), (u8 *)&fw_id_local); // == BUILD_VERSION
    LOG_USER_MSG_INFO(0, 0,"fw_adr+2:0x%x",fw_id_local);
#endif
}

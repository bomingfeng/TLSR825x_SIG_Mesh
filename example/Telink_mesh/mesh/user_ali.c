...
烧录到FLASH_ADR_THREE_PARA_ADR
*/
	flash_read_page(FLASH_ADR_THREE_PARA_ADR,sizeof(product_id_flash),(u8 *)(&product_id_flash));
	LOG_USER_MSG_INFO(0, 0,"FLASH_ADR_THREE_PARA_ADR = product_id_flash:0x%x",FLASH_ADR_THREE_PARA_ADR);
	if(product_id_flash == 0xffffffff){
		return;
	}

	LOG_USER_MSG_INFO(0, 0,"con_product_id:0x%x",con_product_id);
	flash_read_page(FLASH_ADR_THREE_PARA_ADR,sizeof(con_product_id),(u8 *)&con_product_id);
	LOG_USER_MSG_INFO(0, 0,"FLASH_ADR_THREE_PARA_ADR = con_product_id:0x%x",con_product_id);

	idx += sizeof(con_product_id);

	LOG_USER_MSG_INFO(tbl_mac,sizeof(tbl_mac),"tbl_mac = con_product_id", 0);
	flash_read_page(FLASH_ADR_THREE_PARA_ADR+idx,sizeof(tbl_mac),tbl_mac);// read big endian

	endianness_swap_u48(tbl_mac);// change to small endian	/* 更改为小端 */
	LOG_USER_MSG_INFO(tbl_mac,sizeof(tbl_mac),"tbl_mac = change to small endian", 0);
	...
	LOG_USER_MSG_INFO(con_sec_data,SIZE_CON_SEC_DATA,"SIZE_CON_SEC_DATA", 0);
#endif
	return ;
}
...
void set_sha256_init_para_mode(u8 mode)
{
	if(mode){
		...
		LOG_USER_MSG_INFO(tbl_mac,sizeof(tbl_mac),"tbl_mac = con_mac_address", 0);

...
void mesh_node_prov_event_callback(u8 evt_code)	/*EVENT_MESH_NODE_CONNECT*/
{
#if WIN32
#else 
    if( evt_code == EVENT_MESH_NODE_RC_LINK_START ||
		...
		blc_ll_setScanEnable (BLS_FLAG_SCAN_ENABLE | BLS_FLAG_ADV_IN_SLAVE_MODE, 0);/*使能Scan FLAG ,FLAG_ADV。LIB库提供的函数 */
		blc_att_setServerDataPendingTime_upon_ClientCmd(10); /* 设置扫描时间10*10ms.LIB库提供的函数 */

		set_prov_timeout_tick(0); /* 设置超时时间 */

		disable_mesh_adv_filter(); /* 关闭广播过滤 */

		mesh_provision_para_reset();/*复位 provision para 初始状态*/
...
u8 proc_telink_mesh_to_sig_mesh(void)
{
	...
	u32 mesh_type = *(u32 *) FLASH_ADR_MESH_TYPE_FLAG;//从flash 0x73000中读出数据

	LOG_USER_MSG_INFO(0, 0,"mesh_type: 0x%x",mesh_type);
	...
	if(TYPE_SIG_MESH == mesh_type){
		LOG_USER_MSG_INFO(0, 0,"mesh_type:TYPE_SIG_MESH",0);
		return 1;
	}
	...
	{
		LOG_USER_MSG_INFO(0, 0,"mesh_type:%d(TYPE_TLK_BLE_SDK 195 || TYPE_TLK_ZIGBEE 60)",mesh_type);
		ret = 1;
	...
	||(-1 != sig_cust_cap_flag)
		...
		#endif
		#endif
		LOG_USER_MSG_INFO(0, 0,"mesh_type:TYPE_TLK_MESH",0);
		ret = 1;
	}
	if(ret == 1){
		LOG_USER_MSG_INFO(0, 0,"factory_reset",0);
		factory_reset();
	}
	...
	#endif
	{
		...
		LOG_USER_MSG_INFO(0, 0,"set_firmware_type_SIG_mesh:TYPE_SIG_MESH",0);
	}
...
{
	LOG_USER_MSG_INFO(0, 0,"BLT_EV_FLAG_CONNECT:mesh_ble_connect_cb......", 0);
	#if MESH_RX_TEST
	bls_l2cap_requestConnParamUpdate (8, 8, 0, 200);
	#else
	bls_l2cap_requestConnParamUpdate (16, 32, 0, 200);/* 更新连接参数 .LIB库提供的函数*/
	LOG_USER_MSG_INFO(0, 0,"bls_l2cap_requestConnParamUpdate (16, 32, 0, 200)", 0);
	#endif
	
	#if ONLINE_STATUS_EN
    mesh_node_report_init ();
	#endif
	LOG_USER_MSG_INFO(0, 0,"mesh_node_prov_event_callback(EVENT_MESH_NODE_CONNECT)", 0);
	mesh_node_prov_event_callback(EVENT_MESH_NODE_CONNECT);/* mesh节点provision事件回调 */
...
int app_l2cap_packet_receive (u16 handle, u8 * raw_pkt)
{
	...
	u8 *p = raw_pkt + 12;
	u8 conn = 0x40;
	blc_hci_send_data (conn | HCI_FLAG_ACL_BT_STD, p, p[1]);	//dump packet to txFIFO 将数据包转储到 txFIFO
	#endif
...
void ble_mac_init()
{
	...
	LOG_USER_MSG_INFO(mac_read,sizeof(mac_read),"flash_sector_mac_address==0xff:",0);

	u8 ff_six_byte[6] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
	if ( memcmp(mac_read, ff_six_byte, 6) && debug ) {
        ...
        #else
        u8 value_rand[5];
        generateRandomNum(5, value_rand);//随机生成
	...
	LOG_USER_MSG_INFO(tbl_mac,sizeof(tbl_mac),"flash_sector_mac_address!=0xff:",0);
}
...
void mesh_scan_rsp_init()
{
	...
	#endif
	bls_ll_setScanRspData((u8 *)&tbl_scanRsp, rsp_len); /* LIB库提供的函数 */
}
#else
void mesh_ota_reboot_set(u8 type){}
void mesh_ota_reboot_check_refresh(){}
...
{
    get_fw_id();    // must first /* fw_adr+2 = rsp.id.fw_id = 在flash:中的地址 (0 + 2) 数据*/
#if !WIN32

	/* 一。FLASH_GET_JEDEC_ID 包括Capacity
	二。flash_sector_mac_address = CFG_ADR_MAC_512K_FLASH;
		flash_sector_calibration = CFG_ADR_CALIBRATION_512K_FLASH;
		flash_capacity = 读出的Capacity
	*/
	blc_readFlashSize_autoConfigCustomFlashSector(); //读取片上flash配置
	LOG_USER_MSG_INFO(0, 0,"Flash_CapacityDef:0x%x",flash_get_capacity());
	LOG_USER_MSG_INFO(0, 0,"flash_sector_mac_address:0x%x",flash_sector_mac_address);
	LOG_USER_MSG_INFO(0, 0,"flash_sector_calibration:0x%x",flash_sector_calibration);
#endif
    mesh_factory_test_mode_en(FACTORY_TEST_MODE_ENABLE); /* 产测模式，默认开启 */
    user_mesh_cps_init();	/* 设定head.pid head.vid head.crpl ，做用未知？*/

    u8 dev_key_def[16] = DEVKEY_DEF; /* dev_key */
	LOG_USER_MSG_INFO(dev_key_def,sizeof(dev_key_def),"dev_key_def[16] = DEVKEY_DEF:",0);

    memcpy(mesh_key.dev_key, dev_key_def, sizeof(mesh_key.dev_key));
#if (FEATURE_LOWPOWER_EN)
	model_sig_cfg_s.sec_nw_beacon = NW_BEACON_NOT_BROADCASTING;
#else
	model_sig_cfg_s.sec_nw_beacon = SEC_NW_BC_BROADCAST_DEFAULT; /* 安全网络信标 */ // (NODE_CAN_SEND_ADV_FLAG) ? SEC_NW_BC_BROADCAST_DEFAULT : NW_BEACON_NOT_BROADCASTING;
#endif
	LOG_USER_MSG_INFO(0,0,"sec_nw_beacon 安全网络信标:0x%x",model_sig_cfg_s.sec_nw_beacon);

	model_sig_cfg_s.ttl_def = TTL_DEFAULT;  /* 中继次数*/
	LOG_USER_MSG_INFO(0,0,"ttl_def 中继次数:0x%x",model_sig_cfg_s.ttl_def);
	...
	#endif
#endif
	LOG_USER_MSG_INFO(0,0,"frid 友谊:0x%x",model_sig_cfg_s.frid);

	model_sig_cfg_s.gatt_proxy = FEATURE_PROXY_EN ? GATT_PROXY_SUPPORT_ENABLE : GATT_PROXY_NOT_SUPPORT;
	LOG_USER_MSG_INFO(0,0,"gatt_proxy 代理:0x%x",model_sig_cfg_s.gatt_proxy);

	model_sig_cfg_s.node_identity_def = NODE_IDENTITY_SUBNET_SUPPORT_DISABLE;/*Node Identity广播包不知是什么来的？？*/
	LOG_USER_MSG_INFO(0,0,"Node Identity广播包不知是什么来的？:0x%x",model_sig_cfg_s.node_identity_def);

	model_sig_cfg_s.nw_transmit.count = TRANSMIT_CNT_DEF;/* 重传次数和重传间隔的定义 */
	LOG_USER_MSG_INFO(0,0," message重传次数:0x%x",model_sig_cfg_s.nw_transmit.count);

	model_sig_cfg_s.nw_transmit.invl_steps = TRANSMIT_INVL_STEPS_DEF;/* 重传的间隔 = (TRANSMIT_INVL_STEPS_DEF + 1) * 10ms + (0--10) ms. */
	LOG_USER_MSG_INFO(0,0," message重传间隔:0x%x",model_sig_cfg_s.nw_transmit.invl_steps);
	...
	LOG_USER_MSG_INFO(0,0,"中继重传次数:0x%x",model_sig_cfg_s.relay_retransmit.count);
	model_sig_cfg_s.relay_retransmit.invl_steps = TRANSMIT_INVL_STEPS_DEF_RELAY;
	LOG_USER_MSG_INFO(0,0,"中继重传间隔:0x%x",model_sig_cfg_s.relay_retransmit.invl_steps);
	model_sig_cfg_s.relay = FEATURE_RELAY_EN ? RELAY_SUPPORT_ENABLE : RELAY_NOT_SUPPORT;
	LOG_USER_MSG_INFO(0,0,"中继ENABLE:0x%x",model_sig_cfg_s.relay);
	#endif

#if DUAL_VENDOR_EN
	provision_mag.dual_vendor_st = DUAL_VENDOR_ST_STANDBY;
#endif
	mesh_global_var_init_fn_buf(); /* MESH全局变量初始化 */
#if MD_SERVER_EN
	LOG_USER_MSG_INFO(0, 0,"mesh_global_var_init_light_sw.....", 0);
	mesh_global_var_init_light_sw();/*灯的初始化，包括数量，断电是否保持*/
	LOG_USER_MSG_INFO(0, 0,"mesh_global_var_init_light_sw Done", 0);

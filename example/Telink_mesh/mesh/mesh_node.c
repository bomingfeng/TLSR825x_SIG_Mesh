...
u16 ele_adr_primary = 0;    // save in configure server model.保存在配置服务器模型中。
...
void mesh_flash_retrieve()
{
    ...
    mesh_common_retrieve_all();		// should be first, because include model_sig_cfg_s。应该是第一个，因为包括model_sig_cfg_s
...
void mesh_service_change_report()
{
    ...
    #endif
    {
        // force to do the service changes 强制进行服务更改
        #if !WIN32
        u8 service_data[4]={0x01,0x00,0xff,0x00};
        // should keep the service change in the last 应该将服务更改保留在最后
...
#endif
	// read proxy list part 读取代理列表部分
	set_proxy_initial_mode(0);
	// init the heartbeat msg 初始化心跳消息
	init_heartbeat_str();
    // read parameters 读取参数
    mesh_flash_retrieve();	// should be first, get unicast addr from config modle.应该是第一个，从配置modle获取单播加法器。
	mesh_provision_para_init();
	//unprovision beacon send part 取消设置信标发送部分
	beacon_str_init();
	mesh_node_init();
	

	// init the health part 初始化健康部分
	init_health_para();
	mesh_vd_init();
	init_ecc_key_pair();
#if (!FEATURE_LOWPOWER_EN)
